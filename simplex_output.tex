...
El \textit{método de la Gran M} es una técnica para construir una base inicial cuando el problema original contiene restricciones de igualdad o de tipo $\geq$, que no permiten obtener una solución básica factible inmediatamente. Se procede de la siguiente forma:\\[0.2cm]
\begin{itemize}
  \item Se introducen variables artificiales ($a_i$) en cada restricción de tipo $=$ o $\geq$ para formar una base inicial.
  \item En la función objetivo se añade una penalización muy grande $M$ multiplicando cada variable artificial (por ejemplo, $+M a_i$ para maximizar).         De este modo, cualquier solución que deje artificiales básicas con valor positivo tendrá un costo muy alto y será descartada.
  \item Antes de empezar las iteraciones se canoniza la fila 0 (fila de la función objetivo) para que refleje correctamente las contribuciones de $M$;         luego se ejecuta el algoritmo Simplex normal tratando las $M$ como constantes grandes durante los cálculos.
...
Nombre del problema: \textbf{Problema 1 P2}\\[0.3cm]
...
\textbf{Sujeto a:}\\
5 $X_1$ + 6 $X_3$ - $X_4$ \leq 0 \\
$X_1$ + 6 $X_2$ + 4 $X_3$ - $X_4$ \leq 0 \\
4 $X_1$ + 4 $X_2$ + 8 $X_3$ - $X_4$ \leq 0 \\
$X_1$ + $X_2$ + $X_3$ = 1 \\
\\[0.3cm]
...
\toprule
\textbf{$Z$} & \textbf{$X_1$} & \textbf{$X_2$} & \textbf{$X_3$} & \textbf{$X_4$} & \textbf{$s_{1}$} & \textbf{$s_{2}$} & \textbf{$s_{3}$} & \textbf{$a_{1}$} & \textbf{$ $} \\\midrule
1 & 0 & 0 & 0 & -1 & 0 & 0 & 0 & -M & 0\\
0 & 5 & 0 & 6 & -1 & 1 & 0 & 0 & 0 & 0\\
0 & 1 & 6 & 4 & -1 & 0 & 1 & 0 & 0 & 0\\
0 & 4 & 4 & 8 & -1 & 0 & 0 & 1 & 0 & 0\\
...
\toprule
\textbf{$Z$} & \textbf{$X_1$} & \textbf{$X_2$} & \textbf{$X_3$} & \textbf{$X_4$} & \textbf{$s_{1}$} & \textbf{$s_{2}$} & \textbf{$s_{3}$} & \textbf{$ $} \\\midrule
1 & 0 & 0 & -4 & 0 & 0 & 0 & -1 & 4\\
0 & 1 & 0 & 1.2 & 0 & 0 & -0.2 & 0.2 & 0.4\\
0 & 0 & 1 & -0.2 & 0 & 0 & 0.2 & -0.2 & 0.6\\
0 & 0 & 0 & -4 & 1 & 0 & 0 & -1 & 4\\
0 & 0 & 0 & -4 & 0 & 1 & 1 & -2 & 2\\
\bottomrule
\end{tabular}
...
\textbf{$Z$} & \textbf{$X_1$} & \textbf{$X_2$} & \textbf{$X_3$} & \textbf{$X_4$} & \textbf{$s_{1}$} & \textbf{$s_{2}$} & \textbf{$s_{3}$} & \textbf{$ $} \\\midrule
1 & 0 & 0 & -4 & 0 & 0 & 0 & -1 & 4\\
0 & 1 & 0 & 0.4 & 0 & 0.2 & 0 & -0.2 & 0.8\\
0 & 0 & 1 & 0.6 & 0 & -0.2 & 0 & 0.2 & 0.2\\
0 & 0 & 0 & -4 & 1 & 0 & 0 & -1 & 4\\
0 & 0 & 0 & -4 & 0 & 1 & 1 & -2 & 2\\
\bottomrule
\end{tabular}
